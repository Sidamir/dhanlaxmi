...
<head>
  ...
  <title>Home Products | ixiLoan</title>
  <!-- FAVICONS ICON -->
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Home Products</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
            ...
            <div class="card-body">
              ...
              <table id="table" class="table table-bordered table-striped">
                ...
                <tr>
                  <th width="50">ID</th>
                  <th>Title</th>
                  <th>Short Decrepitation</th>
                  <th>Long Decrepitation</th>
                  <th>Home Product Image</th>
                  <th>Main Image</th>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({  
        ...
        "ajax": {
            "url": "<?php echo site_url('Homeproducts_controller/ajax_list')?>",
            "type": "POST"
        },
...
function add_person()
{
    ...
    $('.modal-title').text('Add Director'); // Set Title to Bootstrap modal title
...
function edit_person(id)
{
    ...
    $.ajax({
        url : "<?php echo site_url('Homeproducts_controller/ajax_edit')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('[name="products_desig"]').val(data.products_desig);
            
            $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('Edit Director'); // Set title to Bootstrap modal title
            $('#photo-preview').show(); // show photo preview modal
            if(data.image)
            {
                $('#label-photo').text('Change Photo'); // label photo upload
                $('#photo-preview div').html('<img src="'+base_url+'upload/homeabout/'+data.image+'" class="img-fluid">'); // show photo
            ...
            else
            {
             ...
             if(data.products_img)
            {
                $('#label-photo1').text('Change Photo'); // label photo upload
                $('#photo-preview1 div').html('<img src="'+base_url+'upload/homeabout/'+data.products_img+'" class="img-fluid">'); // show photo
                $('#photo-preview1 div').append('<input type="checkbox" name="remove_photo" value="'+data.products_img+'"/> Remove photo when saving'); // remove photo
            }
            else
            {
...
function save()
{
    ...
    var url;
    if(save_method == 'add') {
        url = "<?php echo site_url('Homeproducts_controller/ajax_add')?>";
    } else {
        url = "<?php echo site_url('Homeproducts_controller/ajax_update')?>";
    } 
...
function delete_person(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('Homeproducts_controller/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
...
<div class="modal fade" id="modal_form" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <h5 class="modal-title" id="exampleModalLongTitle">Home About</h5>
            ...
            <div class="modal-body form">
              <form action="#" id="form" class="form-horizontal">
                    ...
                    <div class="form-body">
                        ...
                        <div class="form-group">
                            <label class="control-label col-md-3">Short Decrepitation</label>
                            <div class="col-md-12">
                                <!-- <input name="desig" placeholder="Description" class="form-control" type="text"> -->
                                <textarea name="desig" rows="4" cols="59"></textarea>
                                <span class="help-block"></span>
                            </div>
                        </div>
                          <div class="form-group">
                            <label class="control-label col-md-3">Long Decrepitation</label>
                            <div class="col-md-12">
                                <!-- <input name="desig" placeholder="Description" class="form-control" type="text"> -->
                                <textarea name="products_desig" rows="4" cols="59"></textarea>
                                <span class="help-block"></span>
                            </div>
                        </div> 
                        <div class="form-group" id="photo-preview">
                            <label class="control-label col-md-3">Home Product Image</label>
                            <div class="col-md-12">
                                (No photo)
                                <span class="help-block"></span>
                            </div>
