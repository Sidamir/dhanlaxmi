...
class Our_vision_controller extends CI_Controller {  
	public function __construct()
	{  
		parent::__construct();
		$this->load->model('Our_vision_model','slider');
	...
	public function index()
	{
		if($this->session->userdata('admin_username') != ''){	 
			...
			$rows['menu_name'] = array('name' => 'our_vision', 'sub_name' => 'main');  
			// $rows['rows_values'] = $this->User_model->selectdata_asc('tbl_shipping_type');
			$this->load->view('back-end/our-vision',$rows); 
		}else{
	...
	public function ajax_list()
	{
		...
		foreach ($list as $person) {
			...
			$row[] = $person->desig;	
			if($person->image) 
				$row[] = '<div class="col-sm-6"><a href="'.base_url('upload/our-vision/'.$person->image).'" target="_blank"><img src="'.base_url('upload/our-vision/'.$person->image).'"  width="150" height="150" /></a></div>';
			else
				$row[] = '(No photo)';
			//add html for action
...
// 		<a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Hapus" onclick="delete_person('."'".$person->id."'".')"><i class="glyphicon glyphicon-trash"></i> Delete</a>
	...
	public function ajax_update()
	{
		...
		{
			if(file_exists('upload/our-vision/'.$this->input->post('remove_photo')) && $this->input->post('remove_photo'))
				unlink('upload/our-vision/'.$this->input->post('remove_photo'));
			$data['image'] = '';
		}
		if(!empty($_FILES['photo']['name']))
		{
			...
			if(file_exists('upload/our-vision/'.$person->image) && $person->image)
				unlink('upload/our-vision/'.$person->image);
			$data['image'] = $upload;
		}
	...
	{ 
		$config['upload_path']          = 'upload/our-vision/';
