...
class Testimonials_controller extends CI_Controller {  
	public function __construct()
	{ 
		parent::__construct();
		$this->load->model('Testimonials_model','slider');
	...
	public function index()
	{
		if($this->session->userdata('admin_username') != ''){	 
			...
			$rows['menu_name'] = array('name' => 'testimonials', 'sub_name' => 'main'); 
	...
	public function ajax_list()
	{
		...
		foreach ($list as $person) {
			...
			$row[] = $person->desig;	
			if($person->image) 
				$row[] = '<div class="col-sm-6"><a href="'.base_url('upload/testimonials/'.$person->image).'" target="_blank"><img src="'.base_url('upload/testimonials/'.$person->image).'"  width="100" height="100" /></a></div>';
			else
				$row[] = '(No photo)';
			//add html for action
	...
	public function ajax_update()
	{
		...
		{
			if(file_exists('upload/testimonials/'.$this->input->post('remove_photo')) && $this->input->post('remove_photo'))
				unlink('upload/testimonials/'.$this->input->post('remove_photo'));
			$data['image'] = '';
		}
		if(!empty($_FILES['photo']['name']))
		{
			...
			if(file_exists('upload/testimonials/'.$person->image) && $person->image)
				unlink('upload/testimonials/'.$person->image);
			$data['image'] = $upload;
		}
	...
	{ 
		$config['upload_path']          = 'upload/testimonials/';
