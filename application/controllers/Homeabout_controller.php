...
class Homeabout_controller extends CI_Controller {  
	public function __construct() 
	{
		parent::__construct();
		$this->load->model('Homeabout_model','slider');
	...
	public function index()
	{
		if($this->session->userdata('admin_username') != ''){	
			...
			$rows['menu_name'] = array('name' => 'home_about', 'sub_name' => 'main');
	...
	public function ajax_list()
	{
		...
		foreach ($list as $person) {
			...
			$row[] = $person->desig;	
			if($person->image) 
				$row[] = '<div class="col-sm-6"><a href="'.base_url('upload/homeabout/'.$person->image).'" target="_blank"><img src="'.base_url('upload/homeabout/'.$person->image).'"  width="150" height="150" /></a></div>';
			else
				$row[] = '(No photo)';
			//add html for action
...
// 		<a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Hapus" onclick="delete_person('."'".$person->id."'".')"><i class="glyphicon glyphicon-trash"></i> Delete</a>
	...
	public function ajax_update()
	{
		...
		{
			if(file_exists('upload/homeabout/'.$this->input->post('remove_photo')) && $this->input->post('remove_photo'))
				unlink('upload/homeabout/'.$this->input->post('remove_photo'));
			$data['image'] = '';
		}
		if(!empty($_FILES['photo']['name']))
		{
			...
			if(file_exists('upload/homeabout/'.$person->image) && $person->image)
				unlink('upload/homeabout/'.$person->image);
			$data['image'] = $upload;
		}
	...
	{
		$config['upload_path']          = 'upload/homeabout/';
