...
class About_controller extends CI_Controller {  
	...
	{
		parent::__construct();
		$this->load->model('About_model','slider');
	...
	public function index()
	{
		if($this->session->userdata('admin_username') != ''){	
			...
			$rows['menu_name'] = array('name' => 'abouts', 'sub_name' => 'main');
			// $rows['rows_values'] = $this->User_model->selectdata_asc('tbl_shipping_type');
			$this->load->view('back-end/about',$rows); 
		}else{
	...
	public function ajax_list()
	{
		...
		foreach ($list as $person) {
			...
			$row[] = $person->desig;	
			// if($person->image) 
			// 	$row[] = '<div class="col-sm-6"><a href="'.base_url('upload/about/'.$person->image).'" target="_blank"><img src="'.base_url('upload/about/'.$person->image).'"  width="150" height="150" /></a></div>';
			// else
			// 	$row[] = '(No photo)';
			//add html for action
...
// 		<a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Hapus" onclick="delete_person('."'".$person->id."'".')"><i class="glyphicon glyphicon-trash"></i> Delete</a>
	...
	public function ajax_update()
	{
		...
		// {
		// 	if(file_exists('upload/about/'.$this->input->post('remove_photo')) && $this->input->post('remove_photo'))
		// 		unlink('upload/about/'.$this->input->post('remove_photo'));
		// 	$data['image'] = '';
		// }
		// if(!empty($_FILES['photo']['name']))
		// {
		...
		// 	if(file_exists('upload/about/'.$person->image) && $person->image)
		// 		unlink('upload/about/'.$person->image);
		// 	$data['image'] = $upload;
		// }
